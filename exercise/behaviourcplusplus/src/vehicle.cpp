...
private:
	const Vehicle *m_ego;
public:
	StateTransition(const Vehicle *ego){
		m_ego = ego;
	}
	vector<string> successor_states(){
		if(m_ego->state.compare("KL") == 0 ){
			return {"KL", "PLCL","PLCR"};
		}
		if(m_ego->state.compare("PlCL") == 0 ){
			return {"LCL", "PLCL"};
		}
		if(m_ego->state.compare("PlCR") == 0 ){
			return {"LCR", "PLCR"};
		}
		if(m_ego->state.compare("lCL") == 0 || m_ego->state.compare("lCR") == 0){
			return {"KL"};
		}
	}

	generate_trajectory(string next_state, map<int,vector < vector<int> > > predictions){
